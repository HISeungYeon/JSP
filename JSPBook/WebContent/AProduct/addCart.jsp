...
<%
	...
	// 2) session객체에 cartlist라는 속성이 있는지 체킹 => session.getAttribute("cartlist")
	// 장바구니 안에는 상품'들'(List<ProductVO>)이 있다.
	ArrayList<ProductVO> list = (ArrayList<ProductVO>)session.getAttribute("cartlist");
	
	// 2-2) cartlist라는 장바구니가 없으면 생성 장바구니를 생성. session.setAttribute("cartlist")
	if(list == null){
		//list는 null이므로 여기서 리스트를 생성하쟈!
	...
	// P1235라는 상품 객체(productVO)의 quantity 멤버 변수의 값을 1증가
	for(int i=0;i<list.size();i++){
		//list는 장바구니 안의 상품목록(P1234, P1235, P1236 등)
		//list => List<ProductVO>
		//list.get(0) => ProductVO
		//list.get(0).getProductId() => P1234 이런식으로 기본키를 가져옴.
...
// 	}
	...
	//4) product.jsp?id=P1235 경로로 되돌아 가자
